...
public class HazyShooter extends SubsystemBase {
    ...
    private static TalonSRX feederTalon;
    ...
    public HazyShooter(){
        ...
        feederTalon = new TalonSRX(RobotMap.HIGHFEEDERTALON);
    }
    ...
    public void shooterSpit(){ //Functions actually used by commands
        ...
        if(getShooterRPM() >= RobotMap.SHOOTERSETPOINT-1000){
            feederTalon.set(ControlMode.PercentOutput, -1);
        }
    }

    public void toggleFeederMove(){
        if(getShooterRPM() >= RobotMap.SHOOTERSETPOINT - 4000){
            feederTalon.set(ControlMode.PercentOutput,-1);
